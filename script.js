...
function showSafetyHall() {
  ...
  dialog(
    ...
    "원활한 안내를 위해 먼저 간단한 시작 안내를 확인하겠습니다.",
    safetyChoices()
  );
}
...
function getKakaoShareImageName() {
  ...
  if (mainKeys.length === 2) {
    ...
    const shareImageOverrides = {
      "clinical-social": "result-clinical-social-v2.png"
    };

    return shareImageOverrides[pairName] || `result-${pairName}.png`;
  }
...
function getKakaoShareDescription() {
  ...
  if (allSameScore || mainKeys.length >= 3) {
    return "제 결과는 여러 정신건강전문요원을 함께 고려해볼 수 있는 유형이에요. 당신에게 맞는 전문요원도 확인해보세요.";
  }

  const names = mainKeys.map((key) => services[key].expert);

  if (mainKeys.length > 1) {
    return `제 결과는 ${names.join(" · ")}를 함께 고려해볼 수 있는 유형이에요. 당신에게 맞는 전문요원도 확인해보세요.`;
  }

  return `제 결과는 ${names[0]} 유형에 가장 가까웠어요. 당신에게 맞는 정신건강전문요원도 확인해보세요.`;
}
